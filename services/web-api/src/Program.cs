...
builder.Services.AddKafkaFlowHostedService(kafka =>
{
    kafka.AddCluster(cluster => cluster
        ...
        .AddConsumer(consumer => consumer
            .Topic(kafkaOptions.Topics.Events)
...
builder.Services
    .AddSingleton<IDeckRepository, DecksProjection>(provider => provider.GetRequiredService<DecksProjection>())
    .AddSingleton<IDeckActivityRepository, DeckActivityProjection>(provider => provider.GetRequiredService<DeckActivityProjection>())
    .AddSingleton<IDeckUserStatsRepository, DeckUserStatsProjection>(provider => provider.GetRequiredService<DeckUserStatsProjection>())
    .AddSingleton<ICardRepository, CardProjection>(provider => provider.GetRequiredService<CardProjection>())
    .AddSingleton<IFeedRepository, FeedProjection>(provider => provider.GetRequiredService<FeedProjection>())
    .AddSingleton<IRepetitionAlgorithm, SpacedRepetitionProjection>(provider => provider.GetRequiredService<SpacedRepetitionProjection>())
